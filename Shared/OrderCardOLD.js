...
const OrderCard = ({ item, select }) => {
  ...
  const fetchOrderItemsData = async () => {
    try {
      ...
      const orderItemPromises = orderItems.map(async (orderItemId) => {
        const response = await axios.get(`${baseURL}/orderItems/${orderItemId}`);
        return response.data;
      });
  
      const orderItemData = await Promise.all(orderItemPromises);
      console.log("Fetched Order Item Data:", orderItemData);
